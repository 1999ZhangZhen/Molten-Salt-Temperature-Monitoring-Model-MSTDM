...
def process_video(video_path, output_folder):
    ...
    while cap.isOpened():
        ...
        fg_mask_2D = cv2.threshold(fg_mask.copy(), 244, 255, cv2.THRESH_BINARY)[1]
        th = cv2.erode(fg_mask_2D, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)), iterations=2)
        fg_mask_2D_fp = cv2.dilate(th, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (8, 3)), iterations=2)
        inverse_fg_mask = cv2.bitwise_not(fg_mask_2D_fp)
        result_image = cv2.bitwise_and(color_image, color_image, mask=inverse_fg_mask)
        ...
        result_eroded = cv2.erode(one_foreground, np.ones((1, 1), np.uint8), iterations=1)
        final_foreground_color = cv2.bitwise_and(color_image, color_image, mask=result_eroded)

        one_foreground_3D = final_foreground_color.copy()

        center_x, center_y = max_contour.mean(axis=0).astype(int).ravel()
        cv2.circle(final_foreground_color, (center_x, center_y), 0, (0, 0, 0), -1)

        final_foreground_color_median = final_foreground_color

        # final_foreground_color_median = cv2.medianBlur(final_foreground_color_median, 5)     # 是否打开中值滤波
        ...
        if not B_whole:
            ...
            color_image_zz = cv2.medianBlur(color_image, 5)
            color_image_zzz = cv2.cvtColor(color_image_zz, cv2.COLOR_BGR2RGB)

            gray_image = cv2.cvtColor(gray_zz, cv2.COLOR_GRAY2RGB)
            axes[0, 1].imshow(color_image_zzz)
...
output_folder = r'D:\Molten salt temperature prediction\testall'
